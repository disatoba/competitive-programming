...
import java.util.*;
/**
 * Java solution implementing Comparable interface
 * verdict Accepted
 * @author Diego Satoba
 */
class Main {
	static class Team implements Comparable<Team>{
		...
		public int compareTo(Team that) {
			...
			if (this.getPoints()>that.getPoints()) return -1;
			if (this.getPoints()<that.getPoints()) return +1;
			// 2. most wins
			if (this.wins > that.wins) return -1;
			if (this.wins < that.wins) return +1;
			// 3. most goal difference
			if (this.getGoalDifference() > that.getGoalDifference()) return -1;
			if (this.getGoalDifference() < that.getGoalDifference()) return +1;
			// 4. most goals scored
			if (this.goalsScored > that.goalsScored) return -1;
			if (this.goalsScored < that.goalsScored) return +1;
			
			// 5. less games played
			if (this.getGamesPlayed() < that.getGamesPlayed()) return -1;
			if (this.getGamesPlayed() > that.getGamesPlayed()) return +1;
	...
	public static void main(String[] args) throws Exception {
		try (BufferedReader in = new BufferedReader(new InputStreamReader(System.in, StandardCharsets.ISO_8859_1));
			...
			for (int N = Integer.parseInt(in.readLine().trim()); N-->0; ) {
				...
				for (int i=0; i<teams.length; i++) out.printf("%d) %s%n", i+1, teams[i]);
				if (N>0) out.println();
			}
		}
	}
